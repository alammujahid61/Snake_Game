...
#image
image=pygame.image.load('image/grounddback.jpg')
image=pygame.transform.scale(image,(width,height)).convert_alpha()
image_1 = pygame.image.load('image/wel_come.gif')
...
def welcome():
    sound('sound/hiss.mp3')
    exit_game=False
    while not exit_game:
        window.blit(image_1,(0,0))
        display('Press 1 to play level 1',black,350,170)
        display('Press 2 to play level 2',black,350,220)
...
def free():
    ...
    while not exit_game:
        if game_over:
            ...
            display('Press Enter to play',black, 20, 235)
            display('Press q to Quit', black, 20, 285)
            ...
            if abs(x_axis-food_x_axis)<6 and abs(y_axis-food_y_axis)<6:
                ...
                if score>int(high_score):
                    high_score=score
                if intial_velocity>=7:
                    intial_velocity=7
            window.blit(image,(0,0))  # image
            display(f'score={str(score)}   High Score={high_score}', red, -2, -5)
