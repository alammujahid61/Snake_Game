...
font=pygame.font.Font(None,40)  # set font None means default font and 55 is the font size
...
def welcome():
    exit_game=False
    image_1 = pygame.image.load('snk.jpg')
    image_1 = pygame.transform.scale(image_1, (width, height)).convert_alpha()
    while not exit_game:
        window.fill(wao)
        window.blit(image_1,(0,0))
        display('Press 1 to play level 1',(25,65,19),215,230)
        display('Press 2 to play level 2',(25,65,19),215,280)
...
def free():
    ...
    while not exit_game:
        if game_over:
            image_2 = pygame.image.load('game_over.jpg')
            ...
            if abs(x_axis-food_x_axis)<6 and abs(y_axis-food_y_axis)<6:
                pygame.mixer.music.load('peep.mp3')
